...
class ResidualFlow(nn.Module):
    ...
    def forward(self, x, u=None, reverse=False):
        ...
        if not reverse:   
            ...
            if self.LDJ:
                ...
                jacobian = jacobian.clone()  # 先复制 Jacobian，避免修改原始数据
    ...
    def _initialize_weights(self):
        # 对网络中的每一层进行 Xavier 初始化
        for module in self.modules():
            if isinstance(module, nn.Linear):  # 针对线性层
                nn.init.xavier_uniform_(module.weight)  # Xavier Uniform 初始化
...
class TrainModel(pl.LightningModule):
    def __init__(self, model, rank, learning_rate=1e-3, lamb=1, path="model_checkpoint_Van"):
        ...
        self.best_val_loss = float('inf')  # 初始化最小验证误差为无穷大
    ...
    def on_train_epoch_end(self):
        if self.trainer.is_global_zero:  # 确保只在主进程执行
            avg_train_loss = self.trainer.callback_metrics.get("train_loss")
            if avg_train_loss is not None:
                self.train_losses.append(avg_train_loss.item())  # 保存到全局列表
                print(f"Epoch {self.current_epoch}: Average Training Loss = {avg_train_loss.item()}")

    # 结束每个验证 epoch 后执行
    def on_validation_epoch_end(self):
        avg_val_loss = torch.stack(self.validation_outputs).mean()  # 计算验证集的平均损失
    ...
    def configure_optimizers(self):
        ...
        return {
            ...
            "lr_scheduler": {
                "scheduler": scheduler,
                "monitor": "val_loss",  # 必须与 log 中的指标名称匹配
            },
            "gradient_clip_val": 1.0,  # 设置梯度裁剪阈值
            "gradient_clip_algorithm": "norm",
        }
    
def normalize_K_spectral_radius(K, target_radius=1.0):
    eigenvalues = torch.linalg.eigvals(K)
    spectral_radius = eigenvalues.abs().max()
    if spectral_radius > target_radius:
        K = K * (target_radius / spectral_radius)
    return K
...
def train_alternating(g_model, H_train, dim, lifted_dim, num_epochs=50, batch_size=64, lr_g=1e-3, save_dir="checkpoints", model_name="g_model"):
    best_loss = float('inf')
    ...
    with torch.no_grad():
        gx = g_model(x_all)[0]
        gy = g_model(y_all)[0]
        K = torch.linalg.lstsq(gx, gy).solution.detach()

    criterion = nn.MSELoss()
    optimizer_g = optim.Adam(g_model.parameters(), lr=lr_g)

    for epoch in range(num_epochs):
        # === Step 1: Fix K, optimize g_model ===
        g_model.train()
        total_loss = 0.0
        for xb, yb in dataloader:
            ...
            loss = criterion(gx_pred, gy)
            loss.backward()
            optimizer_g.step()

            total_loss += loss.item() * xb.size(0)

        # === Step 2: Fix g_model, update K ===
        g_model.eval()
        with torch.no_grad():
            gx_all = g_model(x_all)[0]
            gy_all = g_model(y_all)[0]
            K = torch.linalg.lstsq(gx_all, gy_all).solution.detach()
            K = normalize_K_spectral_radius(K)

        avg_loss = total_loss / len(dataset)
        if avg_loss < best_loss:
            best_loss = avg_loss
            save_model_and_K(g_model, K, save_dir, model_name)
...
if __name__ == '__main__':
    ...
    g_model = load_g_model("model_checkpoint_Van.ckpt", model)
    g_model, K = train_alternating(g_model, H_train, dim, dim+n_feature, num_epochs, batch_size, learning_rate, save_dir="./", model_name="Van")

...
def spectral_norm_g_model(g_model, exclude_name="linear", max_radius=0.99):
    with torch.no_grad():
        for name, module in g_model.named_modules():
            if isinstance(module, nn.Linear) and name != exclude_name:
                ...
                if radius >= max_radius:
                    module.weight.data *= (max_radius - 1e-2) / radius
...
if __name__ == '__main__':
    ...
    g_model = load_g_model("model_checkpoint_Duff.ckpt", model)
    g_model, K = train_alternating(g_model, H_train, dim, dim+n_feature, num_epochs, batch_size, lr_g, lr_K, save_dir="./", model_name="Duff")

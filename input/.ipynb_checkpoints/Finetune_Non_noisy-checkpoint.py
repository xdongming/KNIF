...
def train_alternating(
    g_model, HX_train, HU_train, dim, lifted_dim,
    ...
    x = torch.tensor(HX_train[:, :-1, :], dtype=torch.float32).to(device)
    y = torch.tensor(HX_train[:, 1:, :], dtype=torch.float32).to(device)
    ux = torch.tensor(HU_train[:, :-1, :], dtype=torch.float32).to(device)
    uy = torch.tensor(HU_train[:, 1:, :], dtype=torch.float32).to(device)

    x_all = x.reshape(-1, x.shape[-1])
    y_all = y.reshape(-1, y.shape[-1])
    ux_all = ux.reshape(-1, ux.shape[-1])
    uy_all = uy.reshape(-1, uy.shape[-1])
    dataset = TensorDataset(x_all, y_all, ux_all, uy_all)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

    with torch.no_grad():
        gx_all = g_model(x_all, ux_all)[0]
        gy_all = g_model(y_all, uy_all)[0]
    ...
    for epoch in range(num_epochs):
        ...
        for xb, yb, uxb, uyb in dataloader:
            xb, yb, uxb, uyb = xb.to(device), yb.to(device), uxb.to(device), uyb.to(device)
            optimizer_g.zero_grad()

            gx, ldj_x = g_model(xb, uxb)
            gy = g_model(yb, uyb)[0]
        ...
        g_model.eval()
        with torch.no_grad():
            gx_all = g_model(x_all, ux_all)[0].detach()
            gy_all = g_model(y_all, uy_all)[0].detach()
...
if __name__ == '__main__':
    ...
    g_model = model
    g_model, K = train_alternating(g_model, HX_train, HU_train, dim, dim+n_feature, num_epochs, batch_size, lr_g, lr_K, lamb, save_dir="./", model_name="NP_noisy")
